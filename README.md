...
直接运行 `./make-topics.sh` 回车，按提示在命令行输入一行话题即可（也可把话题作为参数/文件/stdin 传入）。用 `1 2 3` 编号分隔，不编号时会自动按顿号/换行拆。每个话题自动走三选一：
...
4. **改编脚本**（Claude Opus 4.7 / low thinking）：基于深读细节按文章自身节奏拆 **3-5 页**（控成本，最多 5 页/张图，可用 `AIVIDEO_MAX_SLIDES` 调）中文问答短视频脚本，标题统一为「什么是/为什么/意味着什么/财报好不好」这类搜索友好问句；结尾先抛一个与本期话题相关的开放问题引导评论，再带点赞关注
