...
写作要求：
1. **用故事/场景把概念讲透**——正文仍只讲概念、原理、怎么理解；不要写成当日盘面收评，不要写具体个股涨跌，不要写「今天市场涨了多少」
2. 结构建议：
   - 一级标题：与话题标题一致（保留故事钩子问句，禁止改回干巴巴的「X是什么」）
   - 一句话结论（60 字内，先给普通人能懂的后果）
   - 用 1 个生活化场景或「反常现象」引入（买菜/工资/租房/开店/坐过山车等）
   - 再说清概念在算什么、解决什么问题（公式可有，但先人话后公式；举例用虚构简化数字）
   - 常见误读 2–3 条
   - 和相邻概念的区别（若有）
   - 小结：普通人刷财经新闻时怎么用这个概念（不给买卖建议）
...
封面标题要像「反常故事/生活痛点」，不要像金融课目录。
只输出一个严格 JSON 对象，不要 markdown、不要解释。"""

EDU_PICK_USER = """【今天】{today}
【需要选题数量】{count}
{category_hint}
【栏目参考】（概念可属下列栏目，但标题不要写成教材目录）
- basic：财经基础（宏观指标、财务术语、市场常识）——优先「钱不经花/成绩单/失业」等体感话题
- quant：量化入门（风险、回撤、波动等）——优先「为什么回测赚钱实盘亏」「坐高铁还是过山车」等反差
- valuation：估值与计算——优先「公司值多少/账上有钱却没现金」等结果导向，少写纯公式名
...
{reject_block}
【标题与选题要求·流量优先】
1. 每个话题 12–28 字，聚焦**一个**概念；标题必须是问句，但**禁止**以「X是什么」「X怎么算」「X到底是个啥」做主结构
2. 优先句式（按吸引力排序）：
   - 反常/后果：「为什么……却……」「……怎么把……搞崩/砸了」
   - 体感痛点：「钱为啥越来越不经花」「为什么涨了工资却更慌」
   - 对比误解：「光看收益够不够」「成交量大就等于活跃吗」
3. 可埋入可感知物体/场景（工资、房租、奶茶店、过山车、坑、玻璃等），少用纯术语当标题主角
4. 不要当日新闻、不要个股、不要荐股；概念讲解可借用**虚构或常识级**场景，不要冒充今天盘面
5. **科普概念近似永不重复**：CPI/通胀、DCF、最大回撤、夏普、PE/PB 等已讲过的概念禁止再做；换问法也算重复
6. count=1 时选 1 个与历史差异最大、最值得做的**全新**概念即可
7. count≥3 时尽量覆盖 basic / quant / valuation 三类各至少 1 个，且三类概念互不重叠
8. theme_cluster 必须用 edu_ 开头的英文 snake_case，且与已用概念 id 不重复
9. 差示例（禁止）：「PB市净率是什么」「DCF估值怎么算」「因子投资是什么」
10. 好示例（学习句式，勿照抄）：「为什么账上赚钱却掏不出工资」「回测赚钱实盘为啥常亏」「光看收益够不够判断策略」
...
_DICT_TITLE_RE = re.compile(
    r"^(什么是|.+是什么|.{1,12}怎么算|.{1,12}到底是个啥|.{1,12}到底在算什么)"
)


def _looks_like_dictionary_title(title: str) -> bool:
    """拦截「X是什么/怎么算」教材目录式标题（封面点击率差）。"""
    t = re.sub(r"[？?！!。．\s]", "", (title or "").strip())
    if not t:
        return True
    if _DICT_TITLE_RE.match(t):
        return True
    # 「商誉是什么? 为什么…」仍以词典问句开头
    if "是什么" in t[:10] or t.startswith("什么是"):
        return True
    return False
...
def _validate_pick(
    ...
    *,
    ...
    if not title or len(title) < 6:
        return "标题过短或为空"
    if _looks_like_dictionary_title(title):
        return "标题过于像教材目录（禁止「X是什么/怎么算」主结构，请改成故事/后果/反差问句）"
