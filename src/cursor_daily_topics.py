...
【一级标题·封面点击率硬性要求】（大盘报盘槽位除外）
- 一级标题会直接当短视频封面标题，必须是 **12–28 字问句**
- 优先：「为什么突然…」「…意味着什么」「…跟你有啥关系」「…怎么把…砸崩/带火」
- 尽量带**可感知钩子**：具体物体、大数字、反常盘面（独涨/逆势/午后爆发/被抢购）
- **禁止**教材目录式：「X是什么」「X怎么算」「X到底是个啥」
- **禁止**干巴巴报盘句当主标题：「沪指涨X%」「普涨缩量」等（那是大盘槽位的事）
- 差例：「功率半导体为什么集体涨价」可保留「为什么」但要更具体；更好：「近20家厂商一起涨价，半导体在慌什么」
- 好例：「一块玻璃怎么把光模块股砸崩了」「国家队600亿增持能救A股吗」「氢氟酸为什么突然被抢购」
"""
...
【选题偏好】优先选「反常盘面 + 具体催化」：逆势涨停、午后突然爆发、材料被抢购、政策大数字落地等；少选纯指数涨跌复述。

先判断 **唯一** 最值得写的热点（板块异动、政策消息、产业新闻等，如 MLCC、存储芯片、工业气体等），再写一篇 **热点专题**（1500–2500 字），结构：
1. **一级标题**：故事钩子问句（见下方通用标题规则），含板块/事件关键词
2. 约 150 字交代今日大盘背景（3–4 句即可，不重复第一槽的指数报盘）
3. 一句话结论：这条热点今天为什么上头条
4. 热点是什么、产业链位置（小白能懂，先人话后术语）
5. 今天为什么火（盘面事实 + 消息催化，要有数字）
6. 逻辑能否持续（供需/涨价/政策/海外映射）
7. 风险提示 3 条
8. 结语
...
【选题偏好】优先「跟普通人钱包/工作有关」或「超大数字政策」；少选只有机构才关心的续作/配额黑话当主标题。

写一篇 **国内财经新闻分析**（1500–2500 字），结构：
1. **一级标题**：故事/后果问句（见通用标题规则），必须让路人感到「跟我有关」
2. 一句话结论
3. 事件还原（谁、何时、发生了什么）
4. 为什么重要（对产业/普通人/资金面的影响，普通人视角优先）
...
【选题偏好】优先「监管一键关停/出口限制/产业链卡脖子/映射 A 股主线」；少选单一海外产品发布或纯参数对比（国内观众共鸣弱）。

写一篇 **AI 新闻热点分析**（1500–2500 字），结构：
1. **一级标题**：反常/监管/产业链钩子问句（见通用标题规则），少用生僻英文产品名当唯一主角
2. 一句话结论
3. 事件还原（用大白话）
4. 技术或商业上「新在哪」
5. 对产业链/竞争格局的影响（尽量点到与中国/A股相关的映射，若有）
6. 对普通人意味着什么（客观，不夸张）
7. 风险提示或争议点 2–3 条
8. 结语
...
【选题偏好】优先「预期 180 度转弯、暴涨暴跌、关税/大宗商品跟菜篮子有关」；标题要写清「意味着什么/跟咱有啥关系」。

写一篇 **世界财经新闻分析**（1500–2500 字），结构：
1. **一级标题**：故事/传导后果问句（见通用标题规则）
2. 一句话结论
3. 事件还原
4. 传导链条（如何影响汇率、风险偏好、新兴市场等）
5. 与中国的关联（若有，客观简述；尽量落到普通人能懂的一层）
6. 后续观察点 3 条
7. 结语
